...
struct_yolov5_ob_algoResult algoresult_yolov5n_ob;
struct_yolov5_ob_algoResult algoresult_yolofastest_ob;
struct_yolov5_pose_algoResult algoresult_yolov5_pose;
...
static void dp_app_cv_yolov5n_ob_eventhdl_cb(EVT_INDEX_E event)
{
	uint16_t err;
	int32_t read_status;
...
// #endif
	...
	if( g_trans_type == 0 )// transfer type is (UART) 
	{
		cv_yolov5n_ob_run(&algoresult_yolov5n_ob);
	}
	else if( g_trans_type == 1 || g_trans_type == 2)// transfer type is (SPI) or (UART & SPI) 
	{
		#if TOTAL_STEP_TICK
			...
			cv_yolov5n_ob_run(&algoresult_yolov5n_ob);
				...
				xprintf("Tick for TOTAL yolov5N OB:[%d]\r\n",(loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2));					
			#endif	

			if(g_trans_type == 1)//USE SPI
			{
				algoresult_yolov5n_ob.algo_tick = (loop_cnt_2-loop_cnt_1)*CPU_CLK+(systick_1-systick_2);		
			} 		
		#endif

		#if FRAME_CHECK_DEBUG
			hx_drv_spi_mst_protocol_write_sp((uint32_t)&algoresult_yolov5n_ob, sizeof(struct_yolov5_ob_algoResult), DATA_TYPE_META_YOLOV5_OB_DATA);
		#endif
	}
#else 
	#if TOTAL_STEP_TICK
...
#endif
		//clear_alg_hp_rsult
		for (int i = 0; i < MAX_TRACKED_YOLOV5_ALGO_RES; ++i) 
		{
			algoresult_yolov5n_ob.obr[i].bbox.x = 0;
			algoresult_yolov5n_ob.obr[i].bbox.y = 0;
			algoresult_yolov5n_ob.obr[i].bbox.width = 0;
			algoresult_yolov5n_ob.obr[i].bbox.height = 0;
			algoresult_yolov5n_ob.obr[i].confidence = 0;
			algoresult_yolov5n_ob.obr[i].class_idx = 0;
		}
#endif
		//recapture image
...
void app_start_state(APP_STATE_E state)
{
	...
	{
		if(state == APP_STATE_ALLON_YOLOV5N_OB)
		{
			if(cisdp_dp_init(true, SENSORDPLIB_PATH_INT_INP_HW5X5_JPEG, dp_app_cv_yolov5n_ob_eventhdl_cb, 4, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_2X) < 0)
			{
				xprintf("\r\nDATAPATH Init fail\r\n");
				APP_BLOCK_FUNC();
			}
...
#endif
	if(g_use_case == 0) {
		xprintf("yolov5n object detection\n");
#ifdef EN_ALGO
		cv_yolov5n_ob_init(true, true, YOLOV5_OBJECT_DETECTION_FLASH_ADDR);
#endif
	    app_start_state(APP_STATE_ALLON_YOLOV5N_OB);
	}
	return 0;
}
